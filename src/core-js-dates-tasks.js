...
function getNextFriday(date) {
  ...
  let restDays;
  if (currentDay > 5) {
    restDays = 6 - (6 - currentDay);
  } else {
    restDays = 5 - currentDay === 0 ? 7 : 5 - currentDay;
  }
  const resData = restDays + d.getUTCDate();
  d.setUTCDate(resData);
  return d;
}
...
function formatDate(date) {
  ...
  const month = dates.getUTCMonth() + 1;
  const day = dates.getUTCDate();
  let hour = dates.getUTCHours();
  ...
  return `${month}/${day}/${year}, ${hour}:${formatedMinutes}:${formatedSeconds} ${letters}`;
}
...
function getCountWeekendsInMonth(month, year) {
  let totalWeekend = 0;
  const currMounth = month - 1;
  const date = new Date(year, currMounth, 1);
  while (date.getMonth() === currMounth) {
...
function getWeekNumberByDate(date) {
  ...
  let result = 0;
  const nextWeek = new Date(currentYear, 0, 1);
  const daysOffset = (8 - nextWeek.getDay()) % 7;

  if (daysOffset > 0) {
    result += 1;
    nextWeek.setDate(nextWeek.getDate() + daysOffset);
  }
  while (newDate > nextWeek) {
    result += 1;
...
function getNextFridayThe13th(date) {
  ...
  const resData = restDays + d.getDate();
  d.setDate(resData);
  while (d.getDate() !== 13) {
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const startDateParts = period.start
    .split('-')
    .map((part) => parseInt(part, 10));
  const endDateParts = period.end.split('-').map((part) => parseInt(part, 10));
  ...
  const resultStrings = result.map((date) => {
